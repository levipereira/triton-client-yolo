...
def postprocess_coco(num_dets, det_boxes, det_scores, det_classes, img_w, img_h, input_shape, letter_box=True):
    boxes = det_boxes[0, :num_dets[0][0]] / np.array([input_shape[0], input_shape[1], input_shape[0], input_shape[1]], dtype=np.float32)
    scores = det_scores[0, :num_dets[0][0]]
    classes = det_classes[0, :num_dets[0][0]].astype(np.int64)
    ...
    if letter_box:
        if (img_w / input_shape[1]) >= (img_h / input_shape[0]):
            old_h = input_shape[0] * img_w / input_shape[1]
            offset_h = (old_h - img_h) // 2
        else:
            old_w = input_shape[1] * img_h / input_shape[0]
            offset_w = (old_w - img_w) // 2
